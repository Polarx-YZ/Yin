...
class General(commands.Cog):
    ...
    async def botinfo(self, ctx):
        ...
        total_members = 0
        uptime = str(datetime.timedelta(seconds=int(round(time()-start_time))))
        devs = ", ".join(config.get("devs"))
        
        #* In the future I would like this to not include duplicate members
        ...
        embed = discord.Embed(
            title=f"- Bot Stats -",
            description=f"Stats for {config.get("botName")}"
        )
